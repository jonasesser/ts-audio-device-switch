...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"Teufelswerk's Audio Device Switch";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "Teufelswerk's Audio Device Switch";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
	return "Teufelswerk's Audio Device Switch";
#endif
}
...
const char* ts3plugin_author() {
    ...
    return "Jonas Esser | Justus Klopfer";
}

/* Plugin description */
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return "This plugin adds a new Option to switch the audio device for a specific user. The idea was born on the roleplay GTA V server StateV. Where a small group of outlaws teamed up with the Teufel company to work together to counter music copyright issues.";
}
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT,  MENU_ID_CLIENT_1,  "Playback profile: Default",  "deviceDefault.png");
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT,  MENU_ID_CLIENT_2,  "Playback profile: Muted in Stream",  "deviceMuteInStream.png");
